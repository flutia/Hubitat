 ...
 */
metadata {
    definition (name: 'DAWON DNS Smart Plug 10A', namespace: 'flutia', author:'flutia') {
        capability 'Actuator'
...
def configure() {
    ...
    if (prefIsReportSet) {
        ...
        logI("Configuring - intervalMin:${intervalMin}, intervalMax:${intervalMax}, amount of energy change:${minDeltaEnergy}")
    ...
    def cmd = onOffConfig + energyConfig
    return cmd
}

def reportPowerInterval() {
    logD('reportPowerInterval')

    if (prefIsReportSet) {
        def cmdCurrentPower = new hubitat.device.HubAction("he rattr 0x${device.deviceNetworkId} 0x1 0x0702 0x0400", hubitat.device.Protocol.ZIGBEE)
        sendHubCommand(cmdCurrentPower)

        runIn(prefIntervalPower, 'reportPowerInterval')
    }
}
